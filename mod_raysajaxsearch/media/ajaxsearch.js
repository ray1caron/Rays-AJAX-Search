...
console.log('🔍 ajaxsearch.js is loading...');
...
(function () {
  ...
  async function search(query, page = 1) {
    ...
    console.log('AJAX URL from config:', ajaxUrl);
  ...
  function displayResults(data) {
    ...
    if (config.totalResults > 0) {
      ...
      if (elements.resultsCount) {
        ...
        elements.resultsCount.textContent = `Showing ${start}-${end} of ${config.totalResults} results`;
      }

      // Update search time
      if (elements.resultsTime && data.search_time) {
        elements.resultsTime.textContent = `in ${data.search_time.toFixed(2)} seconds`;
      }
  ...
  function getModuleConfig() {
    ...
    console.log('Available Joomla options:', Object.keys(options));

    const moduleId = Object.keys(options).find((key) => key.startsWith('mod_raysajaxsearch_'));

    console.log('Found module config key:', moduleId);

    if (moduleId) {
      const config = Joomla.getOptions(moduleId);
      console.log('Module config object:', config);
      return config;
    }
    ...
    if (moduleElement) {
      return {
        ...
        params: {
          ...
          // Use the correct Joomla component endpoint for AJAX
          ajax_url:
            moduleElement.dataset.ajaxUrl ||
            window.location.origin + '/index.php?option=com_ajaxsearch&format=raw&task=search',
        },
      };
    }
